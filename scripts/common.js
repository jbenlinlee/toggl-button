...
function createProjectSelect(userData, className) {
  ...
  userData.projects.forEach(function (project) {
    var clients = userData.clients.filter(function (elem, index, array) { return (elem.id === project.cid); });
	 var projectName = (clients.length > 0 ? clients[0].name + " - " : "") + project.name;
    select.appendChild(createOption(project.id, project.cid, projectName));
  });
